...
| alignment | Maximum Hits | --maxHits 10 | The maximum number of matches of each read to the reference sequence that will be evaluated. Default value is 10. |
| alignment | Minimum Anchor Size | --minAnchorSize 12 | The minimum anchor size defines the length of the read that must match against the reference sequence. Default value is 12. |
...
| alignment | Number of Threads | --nproc 8 | Number of threads. Default value is 8. |
| alignment | Algorithm Options | --algorithmOptions "-minMatch 12 -bestn 10 -minPctIdentity 70.0" | Pass alignment options through. |
| filter criteria | Maximum Divergence | --maxDivergence 30.0 | The maximum allowed percentage divergence of a read from the reference sequence. Default value is 30.0. |
| filter criteria | Minimum Accuracy | --minAccuracy 70.0 | The minimum percentage accuracy of alignments that will be evaluated. Default value is 70.0. |
| filter criteria | Minimum Length | --minLength 50 | The minimum aligned read length of alignments that will be evaluated. Default value is 50. |
| filter criteria | Score Cutoff | --scoreCutoff 30 | The worst score to output an alignment. |
