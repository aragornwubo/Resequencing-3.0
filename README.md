...
```
dataset create --generateIndices --type SubreadSet my.subreadset.xml subreads1.bam subreads2.bam ...
```
...
| alignment | Algorithm | --algorithm blasr | Select an algorithm from ('blasr', 'bowtie', 'gmap'). (default: blasr) |
| alignment | Maximum Hits | --maxHits MAXHITS | The maximum number of matches of each read to the reference sequence that will be evaluated. (default: 10) |
| alignment | Minimum Anchor Size | --minAnchorSize MINANCHORSIZE | The minimum anchor size defines the length of the read that must match against the reference sequence. (default: 12) |
...
| alignment | No Split Subreads | --noSplitSubreads | Do not split reads into subreads even if subread regions are available. (default: False) |
| alignment | Concordant | --concordant | Map subreads of a ZMW to the same genomic location. |
| alignment | Number of Threads | --nproc NPROC | Number of threads. (default: 8) |
| alignment | Algorithm Options | --algorithmOptions "-minMatch 12 -bestn 10 -minPctIdentity 70.0" | Pass alignment options through. |
| filter criteria | Maximum Divergence | --maxDivergence MAXDIVERGENCE | The maximum allowed percentage divergence of a read from the reference sequence. (default: 30.0) |
| filter criteria | Minimum Accuracy | --minAccuracy MINACCURACY | The minimum percentage accuracy of alignments that will be evaluated. (default: 70.0) |
| filter criteria | Minimum Length | --minLength MINLENGTH | The minimum aligned read length of alignments that will be evaluated. (default: 50) |
...
| for cmp.h5 | For Quiver | --forQuiver | The output cmp.h5 file which will be sorted, loaded with pulse QV information, and repacked, so that it can be consumed by quiver directly. This requires the input file to be in PacBio bas/pls.h5 format, and --useccs must be None. (default: False) |
| for cmp.h5 | Load QVs | --loadQVs | Similar to --forQuiver, the only difference is that --useccs can be specified. (default: False) |
| for cmp.h5 | By Read | --byread | Load pulse information using -byread option instead of -bymetric. Only works when --forQuiver or --loadQVs are set. (default: False) |
...
| miscellaneous | Seed | --seed SEED | Initialize the random number generator with a none-zero integer. Zero means that current system time is used. (default: 1) |
| miscellaneous | Temporary Directory | --tmpDir TMPDIR | Specify a directory for saving temporary files. (default: /scratch) |
...
| read selection and filtering | Reference Window |  --referenceWindow REFERENCEWINDOWSASSTRING, --referenceWindows REFERENCEWINDOWSASSTRING, -w REFERENCEWINDOW | The window (or multiple comma-delimited windows) of the reference to be processed, in the format refGroup:refStart-refEnd (default: entire reference) |
...
| read selection and filtering | Barcode | --barcode BARCODE | Only process reads with the given barcode name. (default: None) |
...
(optional) Input file of contigId:start-end windows in the reference to whitelist for analysis.
...
* __MapQV__
  * MapQV is a single scalar Phred-scaled QV (between 0 and 255) per aligned read that reflects the mapper's degree of certainty that the read aligned to this part of the reference and not some other.
